...
const fetchMovies = async (type, topic, page) => {
  ...
  switch (type) {
    case 'trending':
      Link = `${apiBaseURL}3/trending/movie/day?language=en-US&page=${page}`;      
      break;
    case 'search':
      Link = `${apiBaseURL}3/search/movie?query=${topic}&language=en-US&page=${page}`;
      break;
    case 'id':
      Link = `${apiBaseURL}3/movie/${topic}?language=en-US`;      
      break;
    case 'cast':
    ...
    case 'reviews':
      ...
      break;    
    default: 
      console.log('Error fetching movies, wrong request type:', type);
      break;
  }
